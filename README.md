...
The assistant answers only from a validated local profile, uses typed read-only tools, verifies generated claims against source evidence, and applies input/output privacy guardrails.
...
- Requires source-backed claims and falls back safely when an answer cannot be grounded.
...
`data/profile.json` is the sole runtime source of claims. There is intentionally no vector database, embedding pipeline, durable chat memory, or write-capable tool in v1.
...
A successful response contains opaque request and conversation IDs plus the grounded answer. Validation, rate-limit, provider, and unexpected failures return sanitized `application/problem+json` bodies.
...
The included `Dockerfile` runs Uvicorn on `0.0.0.0:${PORT:-8000}`. For a Railway deployment, configure at least:
