...
2. Attempt structured intent classification as an optimization: `direct_question`, `search_query`, `filter_request`, `summary_request`, `follow_up`, `out_of_scope`, or `adversarial`.
3. If classification is unavailable, incomplete, or low-confidence, route unmistakable resume requests through deterministic universal search; otherwise clarify or fail closed.
